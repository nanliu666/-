    ...
    <div v-else>
      ...
      <ul class="enclosureGroup">
        <li v-for="(item, index) in knowInfo.attachments" :key="index" class="enclosure">
          ...
          />
          <p style="width: 100%; text-align: center">{{ item.fileName }}</p>
        </li>
  ...
  methods: {
    ...
    getData() {
      ...
      getDetailsById({ id: this.applyDetail.formId }).then((res) => {
        ...
        this.knowInfo = res
        this.knowInfo.updateTime = dateConver(this.knowInfo.updateTime)
      })
    },
    // 预览
    previewContent(file) {
    ...
    downLoad(file) {
      ...
      else if (this.needLoadNum) loadNum({ knowledgeId: this.$route.query.id })
      downLoadFile(file)
    },
    // dialog 关闭事件
    close() {
