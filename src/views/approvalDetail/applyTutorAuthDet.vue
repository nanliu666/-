      ...
      >
        ...
        <div>
          <el-row>
            ...
            <el-col :span="8"><span class="name">在职时间：</span>{{ item.workStartTime }} 至
              {{ item.workEndTime }}</el-col>
...
<script>
import { applyTutorDetail } from '@/api/apply'
...
export default {
  ...
  methods: {
    async trainLeaveDetailFn() {
      ...
      this.editExperData.map((j)=>{
        this.editExperData.workStartTime=dateConver(j.workStartTime)
        this.editExperData.workEndTime=dateConver(j.workEndTime)
      })
    }
  }
}
