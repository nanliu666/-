  ...
  <div>
    <div class="details_course_detailed">
          ...
          <ul>
            ...
            <li>
              ...
              <span v-for="(item, index) in courseData.passCondition" :key="index">
                ...
                <span v-show="item == 'b'">考试通过 {{ courseData.passCondition.split(',').length >= 3 ? ',' : '' }}</span>
      ...
      <div v-show="pitch == 2" class="details_course_detailed_pitch2">
        <ul>
          <li v-for="(item, index) in courseData.contents" :key="index" @click="jumpToLearn(item)">
            <div class="tooltip_" style="display:flex;">
              <i>{{ index + 1 }}</i>
              <span style="flex:1">
                <text-over-tooltip
                  ref-name="testName1"
